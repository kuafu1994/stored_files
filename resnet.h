...
Tensor BasicBlock2(Graph* graph, Tensor input, int outChannels, int stride)
{
  ...
  return graph->relu(graph->add(t1, t2));
}
...
Tensor BasicBlock3(Graph* graph, Tensor input, int outChannels, int stride)
{
  ...
  Tensor t1 = graph->conv2d(input, outChannels, 1, 1, 1, 1, 0, 0, true);
  //t1 = graph->conv2d(t1, outChannels, 3, 3, 1, 1, 1, 1, true);
  t1 = graph->conv2d(t1, outChannels, 3, 3, stride, stride, 1, 1, true);
  // t1 = graph->conv2d(t1, outChannels, 1, 1, 1, 1, 0, 0, false);
  t1 = graph->conv2d(t1, 4 * outChannels, 1, 1, 1, 1, 0, 0, false);

  Tensor t2;

  if(stride > 1 || input.dim[1] != 4 * outChannels) {
    t2 = graph->conv2d(input, 4 * outChannels, 1, 1, stride, stride, 0, 0, true);    
  } else {
    t2 = input;
  }
...
//  input.dim[3] = 222;
  ...
  int stride = 1;
  for (int i = 0; i < 3; i++)
    t = BasicBlock3(graph, t, 64, stride);
  for (int i = 0; i < 4; i++) {
    int stride = (i==0) ? 2 : 1;
